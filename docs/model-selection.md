...
- `TOOL_VISION_MODEL` (default `llava-llama3`) for `image_classify`
- `TOOL_STT_MODEL` (default `whisper`) for `speech_to_text`
- `TOOL_IDE_MODEL` (default `qwen3-coder:14b` or `AGENT_MODEL_CODE`) for `code_autocomplete`
- `OLLAMA_EMBED_MODEL` (default `nomic-embed-text`) for semantic search and memory
